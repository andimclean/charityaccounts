...
var loginConfirm = {
	...
	handler : function(req, res) {
		...
		var user = null;
		var session = null;
		step(function findToken() {
			db.logintokens.findOne({
				token : token
			}, this);
	...
	handler : function(req, res) {
		...
		step(function checkAccount() {
			db.transactions.find({
				...
				date : -1,
				id : -1
			}).skip(fromTxn).limit(countTxn).toArray(this.parallel());
			db.transactions.find({
				orgid : req.org._id,
				accid : req.acc.id
			}).count(this.parallel());
...
exports.bind =
		function bindRoutes(app) {
			...
			db =
					mongo.connect(app.get("mongodb"), [ "logintokens", "users",
							"sessions", "organisations", "transactions" ]);
			mw.setDb(db);
			...
			bindRoute(
					'get',
					'/api/getTransactions/:orgid/:accid/:from/:count',
					getTransactions);
